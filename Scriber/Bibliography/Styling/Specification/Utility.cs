...
namespace Scriber.Bibliography.Styling.Specification
{
    ...
    {
        //public static string Manipulate(string text, TextCase textCase)
        //{
        //    text = textCase switch
        //    {
        //        TextCase.Lowercase => text.ToLower(),
        //        TextCase.
        //    }
        //}
        ...
        public static string[] Split(string? value, params char[] c)
        {
            return (value ?? string.Empty).Split(c, StringSplitOptions.RemoveEmptyEntries);
        }

        public static int Count(params bool[] objects)
        {
            return objects.Count(e => e);
        }

        private static class EnumHelpers<TTarget> where TTarget : struct
        {
            private static readonly Dictionary<string, TTarget> Dict = Enum.GetNames(typeof(TTarget)).ToDictionary(x => x, x => (TTarget)Enum.Parse(typeof(TTarget), x), StringComparer.OrdinalIgnoreCase);
...
//        {
//            return @class.AppendMethod("{0} {1} {2}(ExecutionContext {3}, Parameters {4})", accessor, returnType, methodName, Interpreter.CONTEXT_NAME, Interpreter.PARAMETER_NAME);
//        }
...
//                // done
//                return Interpreter.GetLiteral(order);
//            })
//            .ToArray();
...
//        {
//            // context
//            owner.AddCode(Interpreter.CONTEXT_NAME);
...
//            {
//                owner.AddCode("{0}: {1}", parameterName, Interpreter.GetLiteral(value((T)context)));
//            }
//        }
//    }
}
